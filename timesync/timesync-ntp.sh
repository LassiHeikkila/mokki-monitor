...
function init() {
	...
	2>/dev/null chat -t 1 -V '' 'AT+SAPBR=3,1,"Contype","GPRS"' 'OK' > /dev/ttyS0 < /dev/ttyS0
	2>/dev/null chat -t 1 -V '' 'AT+SAPBR=3,1,"APN","internet"' 'OK' > /dev/ttyS0 < /dev/ttyS0
	2>/dev/null chat -t 1 -V '' 'AT+SAPBR=1,1' 'OK' > /dev/ttyS0 < /dev/ttyS0
	2>/dev/null chat -t 1 -V '' 'AT+CNTP="0.fi.pool.ntp.org"' 'OK' > /dev/ttyS0 < /dev/ttyS0
	2>/dev/null chat -t 1 -V '' 'AT+CNTP' 'OK' > /dev/ttyS0 < /dev/ttyS0
}
